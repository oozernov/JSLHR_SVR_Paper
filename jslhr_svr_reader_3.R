...
source("/Users/olaozernov-palchik/Dropbox (MIT)/Annals_SVR/reviews/bf_function.R")
...
all2$DD<-as.factor(all2$DD)
all2$study<-as.factor(all2$study)
#df_2_rdr<-all2%>%filter(grade==2 && study=='reader') #for replication
#excld<-df_2_rdr$ID
#all3<-all2%>%filter(all2$ID!=excld)

#Test for DD Interaction
fit1<-lm(RC~study+DD*(WR+LC+RAN_Letters+Digits), data=all2)
fit2<-lm(RC~study+DD+WR+LC+RAN_Letters+Digits, data=all2)
...
# Test for DD x Grade interaction
...
# Test for Grade interaction

fit5<-lm(RC~grade*(WR+LC+RAN_Letters+Digits), data=all2)
fit6<-lm(RC~grade+WR+LC+RAN_Letters+Digits, data=all2)
...
df_2_rdr<-all2%>%filter(grade==2 & study=='reader') #for replication
#df_3<-all2%>%filter(all2$grade==3 |all2$grade==4)
...
df2_rdr<-read.csv("/Users/olaozernov-palchik/Dropbox (MIT)/Annals_SVR/paper/jslhr/rep_rdr_2nd.csv")
df2_rdr_dys<-df2_rdr%>%dplyr::filter(DD=="DD")%>%
  select(DD,RC,WR,RAN_Letters,LC,Digits)
df2_rdr_typ<-df2_rdr%>%dplyr::filter(DD=="TYP")%>%
  select(DD,RC,WR,RAN_Letters,LC,Digits)

#Dys
fit_2rdr_D<-lm(RC~WR+LC+RAN_Letters+Digits, data=df2_rdr_dys)
summary(fit_2rdr_D)
out_3D<-tidy(fit_2rdr_D)
knitr::kable(out_2rdr)
lm.beta(fit_2rdr_D)
fit_2rdr_D2 <- stepAIC(fit_2rdr_D, direction = "both",steps = 1000)
fit_2rdr_D2$anova
calc.relimp(fit_2rdr_D, type = c("lmg"),
            rela = TRUE)
#Typ
fit_2rdr_T<-lm(RC~WR+LC+RAN_Letters+Digits, data=df2_rdr_typ)
summary(fit_2rdr_T)
out_3D<-tidy(fit_2rdr_T)
knitr::kable(out_2rdr)
lm.beta(fit_2rdr_T)
fit_2rdr_T2 <- stepAIC(fit_2rdr_T, direction = "both",steps = 1000)
fit_2rdr_T2$anova
calc.relimp(fit_2rdr_T, type = c("lmg"),
            rela = TRUE)
...
df2_rdr_typ<-na.omit(rep_rdr2gd%>%filter(DD=="DD")%>%
  select(DD,RC,WR,RAN_Letters,LC,Digits))
df2_rdr_dys<-na.omit(rep_rdr2gd%>%filter(DD=="TYP")%>%
  select(DD,RC,WR,RAN_Letters,LC,Digits))
...
df2_rdr_typ<-na.omit(df2_rdr_typ)
#2 Typ
b2_rdr_typ <- runRegOnAll(df2_rdr_typ) #make sure that there there is one column before RC 
write.csv(b2_typ,"/Users/olaozernov-palchik/Dropbox (MIT)/Annals_SVR/paper/jslhr/jslhr_bf_outputs/Gd2_rdr_typ.csv")

#2 Dys
b2_rdr_dys <- runRegOnAll(df2_rdr_dys)
write.csv(b2_dys,"/Users/olaozernov-palchik/Dropbox (MIT)/Annals_SVR/paper/jslhr/jslhr_bf_outputs/Gd2_rdr_dys.csv")
